...
export const loadCommentAX = (post_id) => {
  return function (dispatch) {
    axios
      .get(`http://3.36.70.96:8080/api/auth/comment/${post_id}`)
...
export const createCommentAX = (post_id, comments) => {
  return function (dispatch) {
    instance
      .post(`http://3.36.70.96:8080/api/auth/comment/${post_id}`, comments)
...
export const updateCommnetAX = (comment_id, commentData) => {
  return function (dispatch) {
    axios
      .patch(
        "http://3.36.70.96:8080/api/auth/comment/" + comment_id,
        commentData
      )
      .then(() => {
...
export const deleteCommentAX = (comment_id) => {
  return function (dispatch, getState) {
    axios
      .delete(`http://3.36.70.96:8080/api/auth/comment/${comment_id}`)
